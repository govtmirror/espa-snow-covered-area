...
# Created Python script to parse the input metadata file, create the needed
# OMF and ODL parameter files, and run the associated DEM-related applications
# to generate a scene-based DEM for the input metadata file.  The metadata
...
class SceneDEM():
    ...
    # the desired fields in the metadata file, and set the associated class
    # variables with the values read for that field.
    #
    # Inputs:
    ...
    #
    # Notes:
    #     It is expected the input metadata file is an LPGS _MTL.txt file
    #         and follows the format (contains the same fields) of those
    #         files.
    #######################################################################
    def parseMeta (self, metafile, log_handler):
...
#            print "    DEBUG: value *%s*" % value.strip('\"')
        ...
        if (self.path == 0) or (self.row == 0):
            msg = 'Error: obtaining path/row metadata fields from: %s' % metafile
            logIt (msg, log_handler)
            return ERROR

        if (self.map_proj != "UTM") and (self.map_proj != "PS"):
            msg = 'Error: obtaining map projection metadata field from: %s.  Only UTM and PS are supported.' % metafile
            logIt (msg, log_handler)
            return ERROR

        if (self.map_proj == "UTM"):
            if (self.utm_zone == 0):
                msg = 'Error: obtaining UTM zone metadata field from: %s' % metafile
                logIt (msg, log_handler)
                return ERROR
        ...
        if (self.map_proj == "PS"):
            if (self.vert_lon_from_pole == -9999.0) or \
               ...
               (self.false_north == -9999.0):
                msg = 'Error: obtaining Polar Stereographic metadata fields (vertical longitude from pole, true scale latitude, false easting, and/or false northing from: %s' % metafile
                logIt (msg, log_handler)
                return ERROR
    ...
    ########################################################################
    # Description: writeParams creates and writes the necessary ODL and OMF
    # parameter files for running retrieve_elevation, makegeomgrid, and
    # geomresample using the parameter values previously obtained.
    #
    # Inputs:
...
END\n' % self.scene_dem
    ...
    ########################################################################
    # Description: createDEM will use the parameter passed for the LPGS
    # metadata file (*_MTL.txt) to parse the necessary fields to create
    # the OMF and ODL files needed for running retrieve_elevation,
    # makegeomgrid, and geomresample in order to generate a scene-based DEM.
    # If a log file was specified, then the output from each application will
    ...
    #   log_handler - open logfile handler (if called from another python
    #       module); if None then the logfile will be opened for logging, if
    #       one exists
    ...
    #######################################################################
    def createDEM (self, metafile=None, logfile=None, log_handler=None,
        usebin=None):
        ...
        close_logfile = False
        if (log_handler == None) and (logfile != None):
            log_handler = open (logfile, 'w', buffering=1)
            close_logfile = True
        msg = 'Processing scene-based DEMs for Landsat metadata file: %s' % metafile
        logIt (msg, log_handler)
        ...
        if not os.path.isfile(metafile):
            msg = 'Error: metadata file does not exist or is not accessible: %s' % metafile
            logIt (msg, log_handler)
            return ERROR
        ...
        if not os.access(metadir, os.W_OK):
            msg = 'Path of metadata file is not writable: %s.  DEM apps need write access to the metadata directory.' % metadir
            logIt (msg, log_handler)
            return ERROR
        ...
        status = self.parseMeta (base_metafile, log_handler)
        if status != SUCCESS:
        ...
        cmdstr = "%sretrieve_elevation %s" % (bin_dir, self.retrieve_elev_odl)
        print 'DEBUG: command: %s' % cmdstr
        ...
        cmdstr = "%smakegeomgrid %s" % (bin_dir, self.makegeomgrid_odl)
        print 'DEBUG: command: %s' % cmdstr
        (status, output) = commands.getstatusoutput (cmdstr)
        logIt (output, log_handler)
        exit_code = status >> 8
        ...
        cmdstr = "%sgeomresample %s" % (bin_dir, self.geomresample_odl)
        print 'DEBUG: command: %s' % cmdstr
        (status, output) = commands.getstatusoutput (cmdstr)
        logIt (output, log_handler)
        exit_code = status >> 8
        ...
        cmdstr = "gdal_translate -of ENVI HDF5:\"%s\"://B01 %s" % (self.scene_dem, self.scene_dem_envi)
        print 'DEBUG: command: %s' % cmdstr
        (status, output) = commands.getstatusoutput (cmdstr)
        logIt (output, log_handler)
        exit_code = status >> 8
        if exit_code != 0:
            msg = 'Error running gdal_translate, which is expected to be in your PATH. Processing will terminate.'
            logIt (msg, log_handler)
            os.chdir (mydir)
            return ERROR
        ...
        logIt (msg, log_handler)
        if (log_handler != None) and (close_logfile):
            log_handler.close()
        return SUCCESS
