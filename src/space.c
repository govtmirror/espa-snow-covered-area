...
)
{
    ...
    if (put_attr_double (sds_file_id, &attr, dval) != SUCCESS)
    {
    ...
    if (put_attr_string (sds_file_id, &attr, struct_meta) != SUCCESS)
    {
...
Value      Description
-----      -----------
ERROR      Error occurred reading the metadata from the HDF file
SUCCESS    Successful completion
...
)
{
    ...
    attr.nval = 1;
    attr.name = SPACE_ORIENTATION_ANGLE_HDF;
    if (get_attr_double (sds_file_id, &attr, dval) != SUCCESS)
    {
